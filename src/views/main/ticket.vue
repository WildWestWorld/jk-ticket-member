  ...
  >
    <template #bodyCell="{ column, record }">
      ...
      <template v-else-if="column.dataIndex === 'duration'">
        ...
        <div
          v-if="
            record.startTime.replace(':', '') >= record.endTime.replace(':', '')
          "
        >
          次日到达
        </div>
  ...
  setup() {
   ...
   // 判断是否过期
    ...
    const handleQuery = (param) => {
      ...
      loading.value = true;
      axios
        .get("/business/dailyTrainTicket/queryList", {
          params: {
            page: param.page,
            size: param.size,
        ...
        .then((response) => {
          ...
          console.warn(data);
          if (data.success) {
            dailyTrainTickets.value = data.result.list;
            // 设置分页控件的值
    ...
    const showStation = (record) => {
      visible.value = true;
      axios
        .get("/business/dailyTrainStation/queryByTrainCode", {
          params: {
            date: record.date,
            trainCode: record.trainCode,
          },
